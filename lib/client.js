...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CSS = `
			...
			.dshProxyHeader{display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap}
			.dshProxyTitle{font-size:14px}.dshProxyHint{color:var(--dsw-alias-label-tertiary);font-size:12px;line-height:1.5}
			.dshProxyMode{min-width:150px;min-height:36px;padding:0 12px;border:1px solid var(--dsw-alias-border-l2);border-radius:9px;background:var(--dsw-alias-bg-layer-2);color:var(--dsw-alias-label-primary);font:inherit;cursor:pointer}
			.dshProxyMode:hover:not(:disabled){border-color:var(--dsw-alias-label-tertiary)}.dshProxyMode:focus-visible,.dshProxyField :focus-visible{outline:2px solid var(--dsw-alias-label-primary);outline-offset:2px}
			.dshProxyMode:disabled{opacity:.6;cursor:default}.dshProxyCheck{display:inline-flex;align-items:center;gap:6px;cursor:pointer}
		...
		function ProxyRow({ scope, t }) {
			...
			return React.createElement('div', { className: 'dshProxyRow' },
				React.createElement('div', { className: 'dshProxyHeader' },
					...
					React.createElement('select', { className: 'dshProxyMode', 'aria-label': t('title'), value: draft.mode,
						disabled: !snapshot.writable || saving, onChange: (event) => changeMode(event.target.value) },
						['direct', 'system', 'manual'].map((mode) => React.createElement('option', { key: mode, value: mode }, t(mode))))),
