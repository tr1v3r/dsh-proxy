...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const { Menu, Tooltip, IconChevronDownOutline14, IconGlobeOutline14 } = require('@deepseek-ai/dsh-client-ui-primitives');
		const NS = 'dsh-proxy';
		const COPY = {
			zh: {
		...
		const CSS = `
			...
			.dshProxyQuick{box-sizing:border-box;flex:none;width:100%;margin:8px 0 0}.dshProxyQuick.rail{width:36px;height:36px;margin:0}
			.dshProxyQuickButton{width:100%;height:42px;display:flex;align-items:center;gap:8px;padding:0 10px 0 8px;border:0;border-radius:12px;background:transparent;color:var(--dsw-alias-label-primary);font:inherit;font-size:14px;cursor:pointer;text-align:left;white-space:nowrap;overflow:hidden}
			...
			.dshProxyQuickButton:disabled{cursor:default;opacity:.6}.dshProxyQuick.rail .dshProxyQuickButton{width:36px;height:36px;padding:0;justify-content:center;border-radius:50%}
			...
			.dshProxyQuickText{min-width:0;overflow:hidden;text-overflow:ellipsis}.dshProxyQuickState{margin-left:auto;color:var(--dsw-alias-label-tertiary);font-size:12px}.dshProxyQuickError{padding:0 8px;color:var(--dsw-alias-state-error-primary);font-size:12px;line-height:18px}
		`;
		...
		function ProxyQuick({ scope, t, wide }) {
			...
			const anchor = React.createElement(Tooltip, { label, delayMs: 500, disabled: open },
				React.createElement('button', {
					...
					React.createElement(IconGlobeOutline14, { size: wide ? 16 : 18 }),
					React.createElement('span', { className: 'dshProxyQuickDot' })),
					wide && React.createElement('span', { className: 'dshProxyQuickText' }, t('quickLabel')),
					wide && React.createElement('span', { className: 'dshProxyQuickState' }, stateLabel)));
			return React.createElement('div', { className: `dshProxyQuick${wide ? '' : ' rail'}`, 'data-mode': ready ? mode : 'loading' },
				React.createElement(Menu, {
