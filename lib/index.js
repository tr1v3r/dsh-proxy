 ...
 * @returns { mode, proxy, noProxy, exportEnv }.
 */
 ...
 */
/** Wrap `proxied` in the shared routing dispatcher only when bypass rules exist. */
function withBypass(proxied, rules) {
	return rules?.length ? new RoutingDispatcher(new Agent(), proxied, rules) : proxied;
}
...
export function buildDispatcher(config) {
	...
	return withBypass(PROXY_AGENT_FACTORIES[protocol](url), config.noProxy);
}
 ...
 */
export function buildSystemDispatcher(detected) {
	...
	if (detected?.httpProxy || detected?.httpsProxy) {
		...
		return withBypass(new EnvHttpProxyAgent({ httpProxy: http, httpsProxy: https, noProxy: '' }), rules);
	}
	if (detected?.socksProxy) {
		return withBypass(new Socks5ProxyAgent(detected.socksProxy), rules);
	}
	return new Agent();
}
...
function createEnvManager() {
	...
	function sync(effective) {
		...
		const noProxy = effective.noProxy?.length ? effective.noProxy.join(',') : undefined;
...
export function createEngine(logger) {
	...
	function apply(config) {
		...
		if (effective.mode === 'system') {
			...
			let dispatcher;
			try {
				dispatcher = buildSystemDispatcher(detected);
			} catch (error) {
